...
@Slf4j
@Component
public class InputYearHandler extends UserRequestHandler {

    private final TelegramService telegramService;
    private final KeyboardHelper keyboardHelper;
    private final UserSessionService userSessionService;

    public InputYearHandler(TelegramService telegramService, KeyboardHelper keyboardHelper, UserSessionService userSessionService) {
    ...
    public boolean isApplicable(UserRequest userRequest) {
        return isTextMessage(userRequest.getUpdate())
                && ConversationState.WAITING_FOR_YEAR.equals(userRequest.getUserSession().getState());
    }
    ...
    public void handle(UserRequest userRequest) {
        ...
        String yearStr = userRequest.getUpdate().getMessage().getText().replaceAll("\\s", "");;
        try {
